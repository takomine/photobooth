    ...
    >
      ...
      {template.frames.map((frame, idx) => {
        ...
        return (
          ...
          >
            <div
              className={`relative h-full w-full overflow-hidden transition-all duration-300 ${isNext
      ...
      {/* Template overlay (on top of background, but below frames due to DOM order) */}
      {template.overlay && (
        <img
  ...
  return (
    <div className="relative w-full h-full bg-slate-800/90">
      ...
      <video
        ...
        muted
        playsInline
        className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-300 ${hasStream ? "opacity-100" : "opacity-0"
          }`}
      ...
      {/* Number overlay */}
      <div
        className={`absolute inset-0 flex items-center justify-center transition-all duration-300 ${isNext ? "bg-emerald-500/10" : "bg-slate-900/30"
          }`}
      >
        <span
          className={`font-black select-none transition-all duration-300 ${isNext
